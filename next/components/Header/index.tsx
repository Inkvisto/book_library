...
import { register, login, unsign } from "../../pages/api/user"
...
const Header = ({ username }: Partial<userData>) => {
    ...
    React.useEffect(() => {
        if (username) setContentHandler(['userLogin', username])
    }, [username])
    ...
    const handleRegisterSubmit = async (e: React.SyntheticEvent) => {
        ...
        setContentHandler(['userRegister', user])
    };
